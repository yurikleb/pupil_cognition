...
class Recorder(BoxLayout):
        ...
        while self.eyeTrackerStatus == "Connected":
            try:
                ...
                if 'pd' in json_data:
                    ...
                    self.pupil_values["d_left"][-1], self.pupil_values["d_right"][-1])
                    # print(json_data)
    ...
    def osc_connect(self):
        ...
        port = int(
            config.get('OSC', 'osc_port'))  # The port defaults to 50020 but can be set in the GUI of Pupil Capture.
...
class SaveDialog(Popup):
    ...
    def __init__(self, pup_data, sen_data, osc_data,
    ...
    def save_csv_file(self, fileName, dataValues):
        with open('datalogs/%s_%s_%s.csv' % (int(time.time()), self.fileNameBox.text, fileName), 'w',
                  newline='') as newFile:
