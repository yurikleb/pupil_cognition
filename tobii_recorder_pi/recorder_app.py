# An App for recording data incoming from Tobii Eye Tracker
# and other sensors on a Raspberry-Pi
...
#Adafurit libraries to interface with sensors
import board
import busio
...
class Recorder(BoxLayout):
    ...
    senStatus = OptionProperty("Disconnected", options=["Connected", "Disconnected", "Connecting"])        
    ...
    def __init__(self,**kwargs):
        ...
        #Window.size = (1000, 800) <-Doesnt work on a Pi
    ...
    def sensor_connect(self):
        ...
        try:
            ...
            self.senStatus = "Connected"
            self.serLog.text = "Connected to Lux Sensor!"
            self.serConnectBtn.disabled = True
            
        except:
            self.senStatus = "Disconnected"
            print("No Sensor Found")
            self.serLog.text = "Could Not Connect"

        #Start data reading thread    
        if (self.senStatus == "Connected"):
            self.get_ser_data_thread = Thread(target = self.get_sensor_data, args=(tsl,))
            self.get_ser_data_thread.daemon = True
            self.get_ser_data_thread.start()


    def get_sensor_data(self,tsl):
        
        print("Sensor Thread Started, Getting Data!")
        sensorValue = 0.0
        
        while self.senStatus == "Connected":
            try:
                if (tsl != 0):
                    sensorValue = tsl.lux
                ...
                print("Can't parse Sensor data")
                self.senStatus == "Disconnected"
            

        print("Sensor Data Reading Thread Stopped!")
