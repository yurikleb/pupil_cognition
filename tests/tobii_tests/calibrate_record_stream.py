...
def get_data_stream(socket):
    ...
    incomingData = {}
    incomingData['TobiiOutput'] = [] 


    while running:
        data, address = socket.recvfrom(1024)
        json_data = json.loads(data)
        incomingData['TobiiOutput'].append(data)
        # if 'pd' in json_data:
        #     print data
        #     # json_data['pd']
    
    print "Saving Data in a Json File."
    with open('data.txt', 'w') as outfile:  
        json.dump(incomingData, outfile)
...
if __name__ == "__main__":
    ...
    try:
        ...
        td_rec = threading.Timer(0, get_data_stream, [data_socket])
        td_rec.start()
